...
from utils.data import load_mpose, random_flip, one_hot, random_noise
...
class Trainer:
    ...
    def get_model(self):
        ...
        transformer = TransformerEncoder(self.d_model, self.n_heads, self.d_ff, self.dropout, self.activation,
                                         self.n_layers)
        ...
        self.name_model_bin = self.config['MODEL_NAME'] + '_' + self.config['MODEL_SIZE'] + '_' + str(
            self.split) + '_' + str(self.fold) + '.h5' 
    ...
    def do_training(self):
        ...
        self.model.save('./saved_models/small',include_optimizer=False)
        ...
        balanced_accuracy = sklearn.metrics.balanced_accuracy_score(tf.math.argmax(tf.concat(y, axis=0), axis=1),
                                                                    y_pred)

        text = f"Accuracy Test: {accuracy_test} <> Balanced Accuracy: {balanced_accuracy}\n"
        self.logger.save_log(text)

        return accuracy_test, balanced_accuracy, history
    ...
    def do_benchmark(self):
        for split in range(1, self.config['SPLITS'] + 1):
            ...
            for fold in range(1):
                ...
                acc, bal_acc, history = self.do_training()
            ...
            np.save(self.config['RESULTS_DIR'] + self.config['MODEL_NAME'] + f'_{split}_balanced_accuracy.npy',
                    bal_acc_list)
